...
setwd("C:\\Users\\singh\\Desktop\\Homework\\1-R Basics")
...
all_week=week1+week2 #to count the total wins or loss of both days
which.max(all_week)  #gives the index of the max value in a vector
#another solution can be as below--
which(all_week==max(all_week))
#index 5 means its a Friday(answer)
overall_gain=sum(all_week)
print(overall_gain)
#overall i lost 84$
...
# try running 6 > 5, What answer is expected and why?
...
#add region of collection. use colnames() function.the regions are US and NON US.
new_hope <- c(460.998, 314.4)
empire_strikes <- c(290.475, 247.900)
return_jedi <- c(309.306, 165.8)
region <- c("US", "non-US")
...
sd(x) #by using function
var(x)#by using function
sd_manual=(sum((x-mean(x))^2)/length(x))^0.5
sd_manual
var_manual=sd_manual^2
var_manual
...
excel_assign=c(31,36,43,20,22,37,33)#from excel file
excel_sd_manual=(sum((excel_assign-mean(excel_assign))^2)/length(excel_assign))^0.5
excel_sd_manual #matches with excel output....
sd(excel_assign) #matches with excel formulae
...
the_quantiles_Asked
IQR=the_quantiles_Asked[3]-the_quantiles_Asked[1]
print(IQR)#ignore the name as 75%

with_added_outlier=append(y,10)#append function can be used
length(with_added_outlier)
boxplot(with_added_outlier)#i can see my 10 as a big outlier..
#***************************************************************
#Good work!! Any doubts on any question. Use the whatsapp group to discuss.
#Enough for this Week.. GO and njoy, Party hard!! Next weeks gonna be
#even more exciting..